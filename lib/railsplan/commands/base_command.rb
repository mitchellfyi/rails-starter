...
module RailsPlan
  module Commands
    # Base class for all RailsPlan CLI commands
    class BaseCommand
      attr_reader :verbose
      ...
      def template_path
        if defined?(Rails) && Rails.respond_to?(:root) && Rails.root
          File.expand_path('lib/templates/railsplan', Rails.root)
        else
          File.expand_path('lib/templates/railsplan', Dir.pwd)
        end
      end

      def registry_path
        if defined?(Rails) && Rails.respond_to?(:root) && Rails.root
          File.expand_path('config/railsplan_modules.json', Rails.root)
        else
          File.expand_path('config/railsplan_modules.json', Dir.pwd)
        end
      end

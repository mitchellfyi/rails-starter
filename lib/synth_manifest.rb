...
# SynthManifest generates and manages the synth.json manifest file
...
class SynthManifest
  ...
  MANIFEST_PATH = File.expand_path('../synth.json', __dir__)
  ...
  # Generate the comprehensive synth.json manifest
  def generate!
    puts "📋 Generating synth.json manifest..." if @verbose
    
    manifest = {
      manifest_version: MANIFEST_VERSION,
      generated_at: Time.now.strftime('%Y-%m-%dT%H:%M:%SZ'),
      synth_cli_version: detect_synth_cli_version,
    ...
    puts "✅ Generated synth.json manifest at #{MANIFEST_PATH}" if @verbose
    manifest
  end
  ...
  def load
    ...
    begin
      ...
      puts "⚠️  Warning: Could not parse synth.json: #{e.message}" if @verbose
      {}
    end
  end
  ...
  def validate!
    puts "🔍 Validating synth.json manifest..." if @verbose
  ...
  def detect_synth_cli_version
    synth_cli_path = File.expand_path('../bin/synth', __dir__)
    return '1.0.0' unless File.exist?(synth_cli_path)
    
    # Try to extract version from the CLI file
    content = File.read(synth_cli_path)
  ...
  def extract_module_hooks(module_name, template_path)
    ...
    if File.exist?(install_path)
      ...
      if content.include?('bin/synth docs')
        hooks[:install][:after_install] << 'bin/synth docs'
      end
    end

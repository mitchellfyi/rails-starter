...
class TestWithHyphensServiceTest < ActiveSupport::TestCase
  def setup
    @user = users(:one)
    @service = TestWithHyphensService.new(@user)
  end
  ...
  test "should not create invalid item" do
    ...
    assert_no_difference('TestWithHyphensItem.count') do
      item = @service.create_item(attributes)
      assert_nil item
    end
  ...
  test "should update item successfully" do
    item = TestWithHyphensItem.create!(user: @user, name: "Original Name")
  ...
  test "should delete item successfully" do
    item = TestWithHyphensItem.create!(user: @user, name: "To Delete")

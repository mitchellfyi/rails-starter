...
after_bundle do
  ...
  belongs_to :prompt_execution, optional: true' do
    say_status :railsplan_ai, "Updated LLMOutput model for PromptTemplate integration"
  end
  ...
  say_status :railsplan_ai, "PromptTemplate versioning and preview system installed successfully!"
  say_status :railsplan_ai, ""
  say_status :railsplan_ai, "Next steps after installation:"
  say_status :railsplan_ai, "1. Run 'rails db:migrate' to create database tables"
  say_status :railsplan_ai, "2. Run 'rails db:seed' to create example templates"
  say_status :railsplan_ai, "3. Visit /prompt_templates to start using the interface"
  say_status :railsplan_ai, "4. Run 'rails test' to verify everything works"
end
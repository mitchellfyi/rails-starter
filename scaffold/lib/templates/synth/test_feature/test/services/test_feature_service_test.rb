...
class TestFeatureServiceTest < ActiveSupport::TestCase
  def setup
    @user = users(:one)
    @service = TestFeatureService.new(@user)
  end
  ...
  test "should not create invalid item" do
    ...
    assert_no_difference('TestFeatureItem.count') do
      item = @service.create_item(attributes)
      assert_nil item
    end
  ...
  test "should update item successfully" do
    item = TestFeatureItem.create!(user: @user, name: "Original Name")
  ...
  test "should delete item successfully" do
    item = TestFeatureItem.create!(user: @user, name: "To Delete")

...
module RailsPlan
  class CLI < Thor
    TEMPLATE_PATH = File.expand_path('../templates/railsplan', __dir__)
    ...
    def add(module_name)
      ...
      %w[app/controllers app/models app/services app/jobs app/mailers app/policies app/queries config lib db spec].each do |sub_dir|
    ...
    def log_module_action(action, module_name, message = nil)
      log_file = Rails.root.join('log', 'railsplan.log')
      FileUtils.mkdir_p(File.dirname(log_file))
      File.open(log_file, 'a') do |f|
        f.puts "[#{Time.now.strftime('%Y-%m-%dT%H:%M:%S%z')}] [#{action.to_s.upcase}] Module: #{module_name} #{" - #{message}" if message}"
      end
    end
